...
dbutils.widgets.text("scraper_name", "ulovdomov", "Scraper Name")
...
mode_sale = "prodej/nemovitosti"
mode_rent = "pronajem/nemovitosti"

# Build the URL based on available parameters
prep_url_sale = BASE_URL + mode_sale
prep_url_rent = BASE_URL + mode_rent
PREP_URL = [prep_url_sale, prep_url_rent]

print(f"Prepared URL: {PREP_URL}")
                ...
                try:
                    ...
                    for listing in page_listings:
                        listings.append({
                            ...
                            "gps_coordinates_raw": listing.get('geoCoordinates')
                        })
...
listings = await scraper.scrap_listings(max_pages=max_pages, per_page=per_page, offer_types=["sale", "rent"])
...
schema = StructType([
    ...
    StructField("gps_coordinates_raw", StringType(), True)
])
