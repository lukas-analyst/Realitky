MERGE INTO realitky.cleaned.property AS target
  USING (
    ...
    COALESCE(listing_details_ulovdomov.pocet_podlazi, -1) :: SMALLINT AS property_number_of_floors,
    COALESCE(listing_details_ulovdomov.patro, -9) :: SMALLINT AS property_floor_number,
    ...
    TRY_CAST(REGEXP_REPLACE(COALESCE(LEFT(listing_details_ulovdomov.uzitna_plocha, LENGTH(listing_details_ulovdomov.uzitna_plocha) -3), '-1'), '[^0-9-]', '') AS DECIMAL(10,2)) AS area_total_sqm,
    TRY_CAST(REGEXP_REPLACE(COALESCE(LEFT(listing_details_ulovdomov.plocha_pozemku, LENGTH(listing_details_ulovdomov.plocha_pozemku) -3), '-1'), '[^0-9-]', '') AS DECIMAL(10,2)) AS area_land_sqm,
    COALESCE(TRY_CAST(REGEXP_REPLACE(left(pocet_pokoju, 1), '[^0-9]', '') AS SMALLINT), -1) AS number_of_rooms,
    CASE
        WHEN listing_details_ulovdomov.rok_vystavby IS null THEN -1
        WHEN listing_details_ulovdomov.rok_vystavby = 0 THEN -1
        ELSE listing_details_ulovdomov.rok_vystavby :: SMALLINT
    ...
    CASE
        ...
        ELSE listing_details_ulovdomov.rok_rekonstrukce :: SMALLINT
    ...
    COALESCE(listing_details_ulovdomov.reserved, False) AS reserved,
