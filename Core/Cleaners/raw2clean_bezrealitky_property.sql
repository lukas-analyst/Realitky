MERGE INTO realitky.cleaned.property AS target
  USING (
    ...
    IF (listing_details_bezrealitky.podlazi LIKE '% z %', REVERSE(TRIM(LEFT(REVERSE(listing_details_bezrealitky.podlazi), 2))), -1) :: SMALLINT AS property_number_of_floors,
    CASE 
    ...
    COALESCE(REGEXP_REPLACE(listing_details_bezrealitky.uzitna_plocha, '[^0-9]', ''), -1) :: DECIMAL(10,2) AS area_total_sqm,
    COALESCE(REGEXP_REPLACE(listing_details_bezrealitky.plocha_pozemku, '[^0-9]', ''), -1) :: DECIMAL(10,2) AS area_land_sqm,
    -1 :: SMALLINT AS number_of_rooms,
    ...
    -1 :: BIGINT AS property_parking_id,
    COALESCE(property_heating.property_heating_key, -1) :: BIGINT AS property_heating_id,
    -1 :: BIGINT AS property_electricity_id,
    -1 :: BIGINT AS property_accessibility_id,
    IF(listing_details_bezrealitky.unnamed_column LIKE '%Balkón%', 1, -1) :: SMALLINT AS property_balcony,
    IF(listing_details_bezrealitky.unnamed_column LIKE '%Terasa%' OR listing_details_bezrealitky.unnamed_column LIKE '%Lodžie%', 1, -1) :: SMALLINT AS property_terrace,
    IF(listing_details_bezrealitky.unnamed_column LIKE '%Sklep%', 1, -1) :: SMALLINT AS property_cellar,
    IF(listing_details_bezrealitky.unnamed_column LIKE '%Výtah%', 1, -1) :: SMALLINT AS property_elevator,
    CASE
    ...
    -1 :: BIGINT AS property_water_supply_id,
    'XNA' AS property_air_conditioning,
    -1 :: BIGINT AS property_gas_id,
    IF(listing_details_bezrealitky.unnamed_column = 'Internet',1, -1) :: SMALLINT AS property_internet,
    ...
    COALESCE(listing_details_bezrealitky.reserved, False) AS reserved,
...
FROM realitky.raw.listing_details_bezrealitky
    ...
    WHERE
      ...
      AND listing_details_bezrealitky.status = 'active'
  ) AS source
