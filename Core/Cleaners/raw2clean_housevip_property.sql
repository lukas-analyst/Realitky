MERGE INTO realitky.cleaned.property AS target
  USING (
    ...
    COALESCE(property_type.property_type_key, -1) AS property_type_id,
    COALESCE(property_subtype.property_subtype_key, -1) AS property_subtype_id,
    COALESCE(listing_details_housevip.pocet_podlazi, -1) AS property_number_of_floors,
    COALESCE(REGEXP_EXTRACT(LEFT(listing_details_housevip.podlazi_umisteni, 3), '(-?[0-9]+)'), -9) AS property_floor_number,
    COALESCE(property_location.property_location_key, -1) AS property_location_id,
    COALESCE(property_construction_type.property_construction_type_key, -1) AS property_construction_type_id,
    ...
    CASE
      WHEN listing_details_housevip.velikost = '5 a více pokojů' THEN 6
      ELSE COALESCE(TRY_CAST(LEFT(listing_details_housevip.velikost, 1) AS INT), -1)
    END AS number_of_rooms,
    COALESCE(listing_details_housevip.rok_vystavby, -1) AS construction_year,
    ...
    COALESCE(property_parking.property_parking_key, -1) AS property_parking_id,
    COALESCE(property_heating.property_heating_key, -1) AS property_heating_id,
    COALESCE(property_electricity.property_electricity_key, -1) AS property_electricity_id,
    COALESCE(property_accessibility.property_accessibility_key, -1) AS property_accessibility_id,
    CASE WHEN listing_details_housevip.balkon IS NOT null THEN 1 ELSE 0 END AS property_balcony,
    CASE WHEN listing_details_housevip.lodzie IS NOT null THEN 1 WHEN listing_details_housevip.terasa IS NOT null THEN 1 ELSE 0 END AS property_terrace,
    CASE WHEN listing_details_housevip.sklep IS NOT null THEN 1 WHEN listing_details_housevip.plocha_skladu IS NOT null THEN 1 ELSE 0 END AS property_cellar,
    CASE WHEN listing_details_housevip.vytah = 'ano' THEN 1 WHEN listing_details_housevip.vytah = 'ne' THEN 0 ELSE -1 END AS property_elevator,
    CASE
