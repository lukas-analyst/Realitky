MERGE INTO realitky.cleaned.property AS target
  USING (
    ...
    COALESCE(REGEXP_REPLACE(listing_details_remax.rok_vystavby, '[^0-9-]', ''), -1) :: INT AS construction_year,
    ...
    COALESCE(listing_details_remax.reserved, False) AS reserved,
    ...
    FROM realitky.raw.listing_details_remax
      ...
      LEFT JOIN realitky.cleaned.property_gas
        ON 
          CASE 
            WHEN listing_details_remax.plyn IN ('Individuální, Plynovod', 'Plynovod') THEN 'Individuální'
