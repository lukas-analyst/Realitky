...
USING(
    SELECT
        ...
        CASE
            WHEN listing_details_remax.property_name LIKE 'Pronájem%' THEN 'rent'
            WHEN listing_details_remax.property_name LIKE 'Prodej%' THEN 'sale'
            ELSE 'XNA'
        ...
        CASE
            WHEN listing_details_remax.cena = 'Cena na vyžádání v kanceláři' THEN 'On demand'
            WHEN listing_details_remax.poznamka_k_cene LIKE '% zahrnuje%' THEN 'With comission'
            WHEN listing_details_remax.poznamka_k_cene LIKE '%č. %' THEN 'With comission'
            WHEN listing_details_remax.poznamka_k_cene LIKE '%v ceně%' THEN 'With comission'
            WHEN listing_details_remax.poznamka_k_cene LIKE '%aukce%' THEN 'Auction'
            WHEN listing_details_remax.poznamka_k_cene LIKE '%+ provize%' THEN 'No comission'
            WHEN LOWER(listing_details_remax.poznamka_k_cene) LIKE '%+ záloh%' THEN 'No comission'
            WHEN listing_details_remax.poznamka_k_cene LIKE '%včetně%' THEN 'With comission'
            ELSE 'XNA'
        END AS price_type,
        COALESCE(listing_details_remax.poznamka_k_cene, 'XNA') AS price_detail,
        listing_details_remax.reserved AS reserved,
        listing_details_remax.status AS status,
        :cleaner AS src_web,
    ...
    WHERE listing_details_remax.del_flag = false
) AS source
