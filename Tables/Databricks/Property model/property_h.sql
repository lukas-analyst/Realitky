...
COMMENT ON COLUMN realitky.cleaned.property_h.property_id IS 'Unikátní identifikátor nemovitosti (UUID).';
...
COMMENT ON COLUMN realitky.cleaned.property_h.address_house_number IS 'Číslo popisné.';
COMMENT ON COLUMN realitky.cleaned.property_h.address_street_number IS 'Číslo orientační.';
COMMENT ON COLUMN realitky.cleaned.property_h.address_averaged_flag IS 'Příznak, zda je adresa průměrována (TRUE/FALSE).';

COMMENT ON COLUMN realitky.cleaned.property_h.ruian_code IS 'Kód RÚIAN (pro propojení s demografickými daty ČSÚ).';
COMMENT ON COLUMN realitky.cleaned.property_h.ruian_confidence IS 'Míra důvěry v přesnost kódu RÚIAN (0-100%).';
...
COMMENT ON COLUMN realitky.cleaned.property_h.reserved IS 'Příznak, zda je nemovitost rezervována (TRUE/FALSE).';
