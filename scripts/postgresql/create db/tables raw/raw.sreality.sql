...
COMMENT ON TABLE raw.sreality IS 'Tabulka pro ukládání nemovitostí z Sreality';
-- Komentáře k tabulce a sloupcům
COMMENT ON COLUMN raw.sreality.pk IS 'Fyzický primární klíč (interní identifikátor řádku)';
COMMENT ON COLUMN raw.sreality.id IS 'Logický identifikátor nemovitosti (např. z URL)';
COMMENT ON COLUMN raw.sreality.data IS 'RAW data nemovitosti ve formátu JSON';
COMMENT ON COLUMN raw.sreality.hash IS 'Unikátní hash kombinace id a data (verzování změn)';
COMMENT ON COLUMN raw.sreality.ins_dt IS 'Datum a čas vložení záznamu';
COMMENT ON COLUMN raw.sreality.upd_dt IS 'Datum a čas poslední změny (aktualizace)';
COMMENT ON COLUMN raw.sreality.del_flag IS 'Označení, zda je záznam historický (TRUE = starý, FALSE = aktuální)';

CREATE INDEX IF NOT EXISTS raw_sreality_id ON raw.sreality (id);
CREATE INDEX IF NOT EXISTS raw_sreality_id_active ON raw.sreality (id) WHERE del_flag = FALSE;
